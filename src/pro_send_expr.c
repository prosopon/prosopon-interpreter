...
{
    assert(pro_expr_get_type(s, t) == PRO_SEND_EXPR_TYPE);
}
...
PRO_INTERNAL pro_expr* pro_send_expr_create(pro_state* s, pro_expr* identifier, pro_expr* msg)
{
    pro_type type = pro_expr_get_type(s, identifier);
    assert(type == PRO_IDENTIFIER_EXPR_TYPE || type == PRO_CONSTRUCTOR_EXPR_TYPE);
    assert(pro_expr_get_type(s, msg) == PRO_MESSAGE_EXPR_TYPE);
